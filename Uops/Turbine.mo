...
Change Log:
   Turbine flow is limited under choked conditions
*  A Gas turbine with two ports
*     Note in this model
*      inlet and outlet (do not permit flow reversal)
*        Rotor dynamics, choke and stonewall conditions handled
*       Set power input to turbine speed adjusts to it.
20th March
      Sign convention for power input and shaft work were made
       consistent with Compressor.(ie. work done on the system is positive) 
*/


  replaceable package TurbMap = TurboMachineMap;
  ...
  MassFlowRate 		mflow_choke;
  Real 			beta(min=0, max=1);
  // Parametric Line Value at operating point
  MassFlowRate 		Mc  ;
  Real 			Nc  ;
  ...
  Power 		power 	 	; // Power output from the turbine
...
equation
  ...
  /* turbine Map Lookup 
        Corrected Speed and Flows */
           Mc = TurbMap.fnMc(state_in.T, inlet.p, inlet.m_flow);
           Nc = TurbMap.fnMc(state_in.T, inlet.p, speed);
           mflow_choke = TurbMap.fnChokedFlow(speed, state_in.T, inlet.p);

  // Pressure Rise across the unit
         outlet.p = inlet.p / pratio;    
         pratio = TurbMap.getPratio (beta, Nc) ;
         eff    = TurbMap.getEff (beta, Nc) ;
         Mc     = TurbMap.getMc (beta, Nc) ;

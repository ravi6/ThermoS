model plant
  ...
  OnOff onoff(pvMin = 0, pvMax = 10e5, mvMin = 0, mvMax = 1000e-3/60, deadBand = 0.5e5);
  Controller pid(pvMin = 0, pvMax = 1000e-3/60, mvMin = 0, mvMax = 100, Kc = 1, Ti = 1, Td = 0, reverseActing = true);
initial algorithm
  tank.T := 300;
  tank.p := 1e5;
  tank.Xi := Air;
equation
  ...
  onoff.sp = 1e5 + 6e5*(1 - exp(-time/10));
  onoff.pv = tank.p;

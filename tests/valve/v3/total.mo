model plant  
  ...
  .ThermoS.Uops.Reservoir src(redeclare package Medium = Gas, p = 1.0001e5, T = 300, Xi = AirComp);
  .ThermoS.Uops.Reservoir sink(redeclare package Medium = Gas, p = 1e5, T = 300, Xi = AirComp);
  .ThermoS.Uops.Valves.RealValve v1(redeclare each package Medium = Gas, cv = 4e-6 / sqrt(0.5e5), tau = 50e-3);
  .ThermoS.Uops.Valves.RealValve v2(redeclare each package Medium = Gas, cv = 4e-6 / sqrt(0.5e5), tau = 50e-3);
    ...
    package Valves  "Contains Valves and its interfaces " 
      ...
      equation
        ...
        if Compressible then
          inlet.m_flow = cv * charF * sqrt(med.d * max(inlet.p, outlet.p)) * sign(inlet.p - outlet.p) * .Modelica.Fluid.Utilities.regRoot(1 - max(prat, 0.5), dpTol);
        else

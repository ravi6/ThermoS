model plant  
/* This simple model uses a IdealGas Mixture Media of N2,O2 and CO2
        and determines gas  Temperature value given enthalpy
        and composition of mixture.

   The issue:
              Casting the problem with 
                    state = setState_phX call fails to converge
                   despite sensible initial T value  (setting opt = 1 will test this)
              Casting the problem with  
                    state.X = ..; state.p = .. ; h = specificEnthalpy(state)
                   works perfectly well.

    Note:    Although both schemes need iterative solution,

             First method uses T_hX function and internal OneNonLinearEquation solver in 
             IdealGases package and I believe that it is failing to bracket 
             the root properly. 

             Second methods does not appear to utilize T_hX function at all.
             Looks as though state.T becomes teared variable. And a more robust
             external solver is employed.

     Resolution:  If my observations are deemed right, the OneNonLinearEquation
                  solver needs to be improved.

*/
  constant Real[3] AirComp = {0.767, 0.233, 0.000};
  .ThermoS.Media.MyGas.ThermodynamicState state(T(start = 300), X(start = .ThermoS.Media.MyGas.reference_X), p(start = 1e5));
  .Modelica.SIunits.Temperature T;
  .Modelica.SIunits.SpecificEnthalpy h(start = 1e5);
  parameter Integer opt = 0;  // default opt works but opt = 1 fails 
equation
  der(h) = 1e5;
  state.T = T;
  if opt == 1 then
    state = .ThermoS.Media.MyGas.setState_phX(1e5, h, .ThermoS.Media.MyGas.reference_X);
  else
    state.X = .ThermoS.Media.MyGas.reference_X;
    state.p = 1e5;
    h = .ThermoS.Media.MyGas.specificEnthalpy(state);
  end if;
end plant;
...
package ThermoS  "A Modelica Package for Process Simulations" 
  package Media  "All fluids that ThermoS knows about" 
    package MyGas  "Specific ideal Gas Mixture" 
      extends .Modelica.Media.IdealGases.Common.MixtureGasNasa(data = {.Modelica.Media.IdealGases.Common.SingleGasesData.N2, .Modelica.Media.IdealGases.Common.SingleGasesData.O2, .Modelica.Media.IdealGases.Common.SingleGasesData.CO2}, fluidConstants = {.Modelica.Media.IdealGases.Common.FluidData.N2, .Modelica.Media.IdealGases.Common.FluidData.O2, .Modelica.Media.IdealGases.Common.FluidData.CO2}, substanceNames = {"Nitrogen", "Oxygen", "CarbonDioxide"}, reference_X = {0.768, 0.232, 0});
    end MyGas;
  end Media;
end ThermoS;
      ...
      package Common  "Common packages and data for the ideal gas models" 
        ...
        partial package MixtureGasNasa  "Medium model of a mixture of ideal gases based on NASA source" 
          ...
          function h_TX  "Return specific enthalpy" 
            ...
            output .Modelica.SIunits.SpecificEnthalpy h "Specific enthalpy at temperature T";
          protected
            Modelica.Media.IdealGases.Common.DataRecord dr;
          algorithm

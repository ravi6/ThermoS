model plant  
  ...
  .ThermoS.Uops.Tanks.Cvolume buffer(redeclare package Medium = Medium, port(m_flow(start = 1.0, nominal = 1e-3)), vol = 1);
initial equation
  buffer.p = 1e5;
  buffer.T = 300;
  buffer.Xi = Xstart;
  ...
  package Uops  "Unit Operations in ThermoS Package" 
    package Valves  "Contains Valves and its interfaces " 
      ...
      equation
        ...
        inlet.m_flow = cv * charF * sqrt(Medium.density(state)) * .Modelica.Fluid.Utilities.regRoot(inlet.p - outlet.p, dpTol);
      end Valve;
    ...
    package Tanks  " Tanks in ThermoS Package" 
      model Cvolume  
        ...
        Medium.MassFraction[state.nXi] Xi;
    ...
    package IdealGases  "Data and models of ideal gases (single, fixed and dynamic mixtures) from NASA source" 
      ...
      package Common  "Common packages and data for the ideal gas models" 
        ...
        partial package MixtureGasNasa  "Medium model of a mixture of ideal gases based on NASA source" 
          ...
          algorithm
            assert(false, "Ravi : " + String(Xfull[1]) + "," + String(Xfull[2]), AssertionLevel.warning);
            hhh := h_TX(200, X);
            assert(false, "EnthL: " + String(hhh), AssertionLevel.warning);
            hhh := h_TX(6000, X);
            assert(false, "EnthH: " + String(hhh), AssertionLevel.warning);
